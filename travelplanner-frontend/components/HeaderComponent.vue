<template>
  ...
  <Disclosure v-else as="nav" class="bg-white" v-slot="{ open }">
        ...
        <div class="flex h-16 justify-between">
          <div class="flex">
            <div class="-ml-2 mr-2 flex items-center md:hidden">
              <!-- Mobile menu button -->
              <DisclosureButton
                  class="inline-flex items-center justify-center rounded-md p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-gray-700">
            ...
            <div class="hidden md:ml-6 md:flex md:items-center md:space-x-4">
              <a v-for="item in filteredNavigation" :key="item.name" :href="item.href"
                 :class="[item.current ? 'bg-gray-800 text-white' : 'text-gray-500 hover:bg-gray-300 hover:text-gray-700', 'px-3 py-2 rounded-md text-sm font-medium']"
    ...
    <DisclosurePanel class="md:hidden">
      <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
        <DisclosureButton v-for="item in filteredNavigation" :key="item.name" as="a" :href="item.href"
                          :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-500 hover:bg-gray-300 hover:text-gray-700', 'block px-3 py-2 rounded-md text-base font-medium']"
      ...
      <div class="border-t border-gray-700 pt-4 pb-3" v-if="isLoggedIn">
        ...
        <div class="mt-3 space-y-1 px-2 sm:px-3">
          <DisclosureButton
              ...
              :key="item.name"
              class="block w-full rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-100 hover:text-white"
